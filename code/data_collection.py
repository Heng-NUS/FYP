...
class archive(object):
	...
	def to_json(self, out_dir, out_buffer):
	...
	def filter_tweets(self, path, out_dir, ignore_geo = True, language = 'en'):
		'''select all useful tweet from a single json file'''
		name = path.split('/')[-1]
		months = dict(Jan='01', Feb='02', Mar='03', Apr='04', May='05', Jun='06', Jul='07', Sept='09', Oct='10', Nov='11', Dec='12')
		retweet = 0
			...
			with open(path, 'r', encoding='utf-8') as file:
				...
				for f in file:
					...
					if 'text' in data and (ignore_geo or data['user']['geo_enabled']):
						...
						if re.search(self.in_rule, text, flags=re.IGNORECASE):
							#filtering by verbal list
							location = data['user']['location']
							coordinates = data['coordinates']
							place = data['place']
							time_split = data['created_at'].split()
							date = time_split[-1] + '/' + months[time_split[1]] + '/' + time_split[2]
							created_at = date
							...
							if size_count >= max_buffer_size:
								self.to_json(out_dir, out_buffer)
								size_count = 0
								out_buffer.clear()
						del data
					else: continue
	...
	def filter_dirs(self, dir_path, out_dir, ignore_geo=True):
		'''filter all files in directroy'''
		...
		retweet = 0
		for file_path in path_list:
			(ori, ext, ret) = self.filter_tweets(file_path, out_dir, ignore_geo)
			ori_count += ori
			extracted += ext
			retweet += ret
		print(dir_path, "ori:", ori_count, "ret:", retweet, "extracted", extracted)
	
class text_pro(object):
	"""preprocoss twitter textual data"""
	def __init__(self):
				...
				if line != "\n":
					try:
						...
						if not re.search(rule, text, flags=re.IGNORECASE) or re.search(ex_rule, text, flags=re.IGNORECASE):
							...
							print(text + "\n")
							label = input("Relevant to ful? (Enter q to quit) ")
						...
						out_file.flush()
						data_count += 1
						print("data_number:", data_count, 'positive:', positive, '\n')
...
input_path = '/Volumes/NonBee5/Twitter/2018/10/06'
