...
def weight_matrix(target_vocab, word_embeddings):
    '''creat a look-up table for word embeddings'''
    matrix_len = len(target_vocab)
    embedding_size = word_embeddings.dim
...
class TextCNN(nn.Module):
    def __init__(self, config, word_embeddings):
        ...
        vocab_size, emb_size = word_embeddings.shape
...
class EBTM(nn.Module):
    ...
    def run_epoch(self, bi_idx, epoch):
        ...
        recons = []
        if epoch % 5 == 0:
            torch.save(self.state_dict(), 'ebtm_parameters.pkl')
        s_time = time.clock()
