...
\end{enumerate} 
In reality, social media topics are changing everyday. The content and number of healthcare topics change dynamically. Topic models listed above are relatively static. The number of topics K is a hyper-parameter that should be given before training them. 

Topic model can contribute to the second requirement of our model, but it cannot solve the first one. K-means is one simple and basic unsupervised clustering algorithm, it groups samples based on Euclidean distance. One inspiration from it is that documents of same topic should be ``close'' to each other. This bring a question: how to represent document? The most common method used in NLP is transforme the original text data into vector 
